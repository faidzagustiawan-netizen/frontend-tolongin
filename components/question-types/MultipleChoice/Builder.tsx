...
export default function MultipleChoiceBuilder({ comp, onChange }: BuilderProps) {
  return (
    <div className="space-y-4 pt-2">
      <label className="text-xs text-muted-foreground font-bold block uppercase tracking-wider">Pilihan Jawaban</label>
          ...
          <div key={optIdx} className="flex items-center gap-3 group">
            ...
            <input
              ...
              className={`flex-1 bg-bg/50 border rounded-xl px-4 py-2.5 text-sm transition-colors focus:outline-none focus:border-emerald-500 ${
                opt.isCorrect ? 'border-emerald-500/50 text-white' : 'border-border text-muted-foreground'
              }`}
            ...
            <button 
              ...
              className="p-2.5 text-muted-foreground hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
              title="Hapus opsi"
            >
