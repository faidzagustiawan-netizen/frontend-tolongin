...
const PodiumItem = ({ item, rank, delay }: { item: any; rank: number; delay: number }) => {
  ...
  return (
    <Link href={`/talents/${item.slug || item.userId}`} className="flex flex-col items-center justify-end flex-1 group">
      <motion.div
      ...
      >
        <div className="relative mb-4 flex flex-col items-center">
          ...
          <div className={`relative rounded-full border-4 ${rank === 1 ? 'border-yellow-400 h-20 w-20 sm:h-24 sm:w-24' : rank === 2 ? 'border-gray-300 h-16 w-16 sm:h-20 sm:w-20' : 'border-[#CD7F32] h-14 w-14 sm:h-16 sm:w-16'} overflow-hidden shadow-xl z-10 bg-background flex items-center justify-center group-hover:scale-105 transition-transform`}>
            {item.avatarUrl ? (
        ...
        <div className={`w-full max-w-[120px] sm:max-w-[160px] mx-auto rounded-t-2xl border-t border-l border-r border-foreground/10 ${height} flex flex-col items-center justify-start pt-4 px-2 text-center bg-gradient-to-t from-dark-bg ${rank === 1 ? 'to-yellow-400/20' : rank === 2 ? 'to-gray-400/20' : 'to-[#CD7F32]/20'} group-hover:brightness-110 transition-all`}>
          <h4 className="font-bold text-foreground text-xs sm:text-sm truncate w-full group-hover:text-emerald-400 transition-colors">{item.fullName}</h4>
