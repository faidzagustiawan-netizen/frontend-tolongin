...
export default function ChallengeSubmissionsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return <span className="px-3 py-1 rounded-full bg-gray-500/10 text-muted-foreground text-xs font-semibold w-fit">{status}</span>;
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Daftar talenta yang telah mengumpulkan pekerjaan untuk studi kasus ini.</p>
      </div>
        ...
        <div className="p-4 border-b border-border flex items-center justify-between bg-foreground/5">
          <div className="relative w-full max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-background border border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-emerald-500/50 transition-colors"
              value={searchTerm}
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-muted-foreground">Memuat data submisi...</p>
            </div>
          ) : filteredSubmissions.length > 0 ? (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-background/50 border-b border-border">
                  <th className="px-6 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Kandidat</th>
                  <th className="px-6 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Status Penilaian</th>
                  <th className="px-6 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Tanggal Submit</th>
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-foreground">{sub.talent.fullName}</p>
                          <p className="text-xs text-muted-foreground truncate max-w-[150px]">{sub.talent.skills?.slice(0, 2).join(', ')}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="text-lg font-bold text-foreground mb-2">Belum ada kandidat</h3>
              <p className="text-sm text-muted-foreground max-w-md">Belum ada kandidat yang mengumpulkan solusi untuk studi kasus ini, atau nama tidak ditemukan.</p>
            </div>
          )}
        </div>
      </div>
